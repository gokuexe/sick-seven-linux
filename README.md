...
```bash
cd sick_seven
flutter create . --platforms=android,ios   # genera android/ e ios/
flutter pub get
flutter run
```

El paso `flutter create .` es necesario porque el repo trae solo `lib/`,
`assets/` y el `pubspec.yaml`. No pisa esos archivos: solo agrega las carpetas
...
flutter build appbundle --release    # para subir a Play Store
```
